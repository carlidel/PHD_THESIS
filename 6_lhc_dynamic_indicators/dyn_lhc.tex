\chapter{Analysis of LHC lattices with dynamic indicators}
...
To further highlight this characteristic of $FMA$, we can observe how the indicator is particularly sensitive to the presence of longitudinal dynamics. Indeed, the longitudinal dynamics couples with the transverse one also introducing tunes modulation via a finite value of the chromaticity. In Fig.~\ref{fig:fma_vs_fli}, we present $FMA$ evaluated on the best seed for three values of $\zeta_0$, and we compare the resulting structures with those highlighted by $FLI^{WB}$. Two essential features can be observed. The first one is that the chaotic regions at the border of the stable region resemble very much for the two indicators, which show that their behaviour is similar. The second one is a strong difference between the two indicators in the region close to the vertical axis. Such a difference grows significantly as a funcrtiWe observe how larger values of $\zeta_0$, leading to stronger tunes modulation, cause larger differences in the structures highlighted by $FMA$ and $FLI^{WB}$. This is due to the fact that stronger longitudinal dynamics leads to stronger modulation effects in the transverse tunes, which ultimately may lead to cases of regular orbits with varying fundamental frequencies.   

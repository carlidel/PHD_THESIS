...
If an analytical expression for $DM(\vb{x}, n)$ is available, a direct application of Eq.~\ref{eq:linear_response} is possible for evaluating $\boldsymbol{\Xi}_{n}(\mathbf{x})$. If this is not the case, it is possible to estimate the linear response with the consolidated ``shadow particle'' method~\cite{}, which consists in approximating the limit $\epsilon\to 0$ with an $\epsilon$ of small finite value, performing the orbit tracking of both $\vb{x}_n$ and $\vb{y}_n$, and then evaluating the estimate $\boldsymbol{\Xi}_{n}(\mathbf{x})=\frac{\mathbf{y}_{n}-\mathbf{x}_{n}}{\epsilon}$.

When tracking $\vb{x}_n$ and $\vb{y}_n$, it is also necessary, every $\tau$ iteration, to evaluate the displacement vector $\vb{y}_n - \vb{x}_n$, and modify $\vb{y}_n$ so that the new displacement vector has module $\epsilon$ and the same direction. This is done to avoid spurious effects in the FLI evaluation that might be caused by excessive orbit drifting~\cite{}. For notation clarity, we denote this periodically modified $\vb{y}$ as $\vb{y}'$, which then reads
\begin{equation}
\begin{aligned}
    \vb{y}'_0 &= \vb{y}_0 \,; \\
    \vb{y}'_i &= DM(\vb{x}_{i\tau - 1}, i\tau - 1)\cdots DM(\vb{x}_{(i-1)\tau}, (i-1)\tau)\left[\vb{x}_{(i-1)\tau}+\epsilon\,\frac{\vb{y}'_{i-1}-\vb{x}_{(i-1)\tau}}{\norm{\vb{y}'_{i-1}-\vb{x}_{(i-1)\tau}}}\right]\,, \qquad 0 \leq i \leq n/\tau \,.
\end{aligned}
\end{equation}

To integrate $\vb{y}_n'$ into the FLI expression, it is possible to take advantage of the properties of the logarithm in Eq.~\eqref{eq:base_fli}, and express FLI$_n/n$ as an average along the trajectory $\vb{x}_n$, leading to
\begin{equation}
    \frac{\text{FLI}_n(\vb{x})}{n} = \sum_{i=1}^{n/\tau} \frac{\log{\norm{\frac{\vb{y}_{i}' - \vb{x}_{i\tau}}{\epsilon}}}}{n}\,.
    \label{eq:fli_mean}
\end{equation}


This estimation method has been widely used in literature due to its simplicity in implementation, however, in multiple studies ~\cite{} it has been highlighted in multiple studies~\cite{} how the choice of both the value of $\epsilon$ and the time interval $\tau$ between displacement resets to $\epsilon$ can significantly alter the final evaluation of $\boldsymbol{\Xi}_{n}(\mathbf{x})$ and, consequently, FLI$_x(\vb{x})$.
...
The $GALI^{(k)}$ indicator can be chosen for different values of $k$, namely, $2 \leq k \leq 2N$, where $N$ is the number of degree of freedom in the system which, in our case, is $N=3$. For $k=6$ we have the natural choice for the linearly independent initial displacements corresponding to the six 

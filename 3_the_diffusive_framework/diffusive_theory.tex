...
If instead we are considering a noise characterized by a specific spectrum, also referred to as coloured noise, we can consider its Fourier expansion, which reads
\begin{equation}
...
As the perturbation causes the breakdown of the KAM tori in certain regions of the phase-space, we observe an overlapping of a regular dynamics and a random dynamics characterized by a small Lyapunov exponent (i.e.\ a small chaotic character), depending on the initial condition. We set as working hypotheses that it is possible to achieve a phenomenological description of the dynamics of the system by means of a stochastic perturbed Hamiltonian description in the form of
\begin{equation}
...
Specifically, the ``noise'' depends on the angle variables that can be regarded as a fast variable, which are independent on a timescale much shorter than the diffusion timescale. To achieve this, let us consider the slow variables
\begin{equation}
    \theta = \phi - \Omega(I) t \,,
\end{equation}
...
Let us now estimate the variance of angle variables using the dominant term for an evolution time $T_\theta$ under the assumption that the action variables can be considered constant during such a time interval. We obtain the following
\begin{multline}
...
From this final expression, we have that the evolution of the distribution function $\rho(I,\tau)$ on the diffusion timescale is well approximated by the solution of the Fokker-Planck equation
\begin{equation}
...
Now we are interested in having an effective generic functional form for $D(I)$. To achieve this, we follow the construction presented in~\cite{Bazzani:2019lse}, and the references therein.
...
% To achieve this, let us consider the following change of variables:
% \begin{equation}
% 	\begin{aligned}
% 		\phi &\to \phi - \Omega \tau\,, \\
% 		t &\to t - \tau\,.

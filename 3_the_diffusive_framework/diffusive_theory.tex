...
Let us consider a simple example of angular diffusion. Let \(x\) be an angular variable defined over the interval \([0,1]\). Let us define then
\begin{equation}
	\begin{aligned}
		\dot{x} &= \omega + y \mod{1}\,, \\
		\dot{y} &= \epsilon\xi(t)	\,.
...
\end{equation}
where we took advantage of the fact that \(\rho_1(\phi, I; 0)=0\). If we now multiply both members by \(\xi(t)\), and compute the average over all the noise realisations, we have, for the case of a Wiener noise:
\begin{equation}
...
\end{equation}
and take the angular average of this last equation. The second term in the r.h.s\ will then become zero, while the first term constitues the expression of \(D(I)\), which now has to be integrated over the whole torus.

\subsection{Averaging principle for generic stochastic Hamiltonians}

We now generalise this last result to stochastically perturbed Hamiltonian systems at higher dimensions. Let us start by considering a generic Hamiltonian system
\begin{equation}
...
\end{multline}
from here, we can combine the resulting two integrals toghether and replace their arguments using the expression for $\Delta I_j(T)$ given in Eq.~\eqref{eq:averaging-generic-hams}, and obtain
\begin{equation}
...
If then we assume that the distribution of \(\phi\) is uniform, we can average on both the noise realisations and on $\phi$. We then can consider the average of the stochastic dynamics of \(I\), which now reads
\begin{equation}
	\begin{aligned}
...
\end{equation}
where \(\hat{\ham_1}=\ham_1/\norm{\ham_1}\) and \(\hat{\xi}_l\) are identical independent distributed random variables with zero mean value and unitary variance. We can interpret the quantity \(T\lambda\norm{\ham_1}^2\) as the time step \(\Delta\tau\) of the diffusion time, we highlight however that it has a dimension of an action. We will then require that \(T\) should be sufficiently long in order to consider the angles relaxing to a uniform distribution and the noise decorrelated, but \(\norm{\ham_1}^2\) has to be so small that the actions do not evolve in a time \(T\). 
...
We are now interested in having an effective generic functional form for $D(I)$. To archive this, we follow the construction presented in~\cite{}, and references therin.
...
\end{equation}
If we then substitue this relation in the previos equation, we obtain a Nekhoroshev-like estimate that reads
\begin{equation}

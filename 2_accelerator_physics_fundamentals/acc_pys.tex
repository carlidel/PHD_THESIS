...
\begin{figure}[t]
    ...
    \caption{Sketch of the different $V$ provided at particles with different $\phi$ for an accelerating bucket, and consequent phase evolution respectively in the regime below transition energy (left, $\eta>0$), and in the regime above transition energy (right, $\eta<0$). $\phi_s$ and $\phi'_s$ represent the synchronous particle phase at $\eta>0$ and $\eta<0$, respectively. $\phi_1$ and $\phi_2$ are respectively the phase of a particle with higher and lower longitudinal momentum, the same goes for $\phi'_1$ and $\phi'_2$. The frequency of the RF voltage provides a restoring force towards $\phi_s$, causing a different effect on the phase depending on the value of $\eta$.}
    \label{fig:long_1}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{Sketch of the different $V$ provided at particles with different $\phi$ for a stationary bucket below transition energy. The synchronous particle $\phi_s$ receives no energy from the RF voltage, while particles at different phase receive a restoring force towards $\phi_s$. The frequency of the RF voltage (black curve) describes a potential well (red curve).}
    \label{fig:long_2}
\end{figure}
